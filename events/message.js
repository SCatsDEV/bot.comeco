...
let mention = [`<@${client.user.id}>`, `<@!${client.user.id}>`]      
            mention.find(mention => {
                if(message.content === mention) {
                  ...
                  .setColor('#f7faff')
                  .setDescription(`**${message.author.username}**, Para mais informação tenta **/ajuda**!`)
...
if (!message.member.hasPermission("ADMINISTRATOR")){
  if (inviteLink.some(word => message.content.toLowerCase().includes(word))) {
    ...
    return message.reply("má escolha de palavras!")
  ...
  if (!timestamps.has(member.id)) {
    ...
    if (now < expirationTime) {
      ...
      return message.channel.send(`Precisas de esperar **${timeLeft.toFixed(1)}** segundo/s para executar de novo este comando.`);
    }
